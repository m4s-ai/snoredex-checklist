...
export function validateProvenance(value: unknown, catalogue: CatalogueSnapshot): boolean {
  ...
  if (value.mode === 'synthetic-fixture') {
    return value.sourceCommit === 'synthetic-fixture' && value.lock === null;
  }
  if (
    value.mode !== 'pinned-snapshot' ||
    typeof value.sourceCommit !== 'string' ||
    !/^[0-9a-f]{40}$/u.test(value.sourceCommit) ||
    typeof value.catalogueFingerprint !== 'string' ||
    !/^sha256:[0-9a-f]{64}$/u.test(value.catalogueFingerprint) ||
    value.catalogueFingerprint !== catalogue.meta.catalogueFingerprint ||
    typeof value.catalogueByteSha256 !== 'string' ||
    !/^sha256:[0-9a-f]{64}$/u.test(value.catalogueByteSha256) ||
    typeof value.catalogueByteLength !== 'number' ||
  ...
  return (
    ...
    lock.sourceRepository === catalogue.meta.sourceRepository &&
    lock.producerRevision === value.sourceCommit &&
    isArtifactUrl(lock.artifactUrl) &&
    lock.contractVersion === value.contractVersion &&
    lock.catalogueFingerprint === value.catalogueFingerprint &&
    lock.catalogueByteSha256 === value.catalogueByteSha256 &&
    lock.catalogueByteLength === value.catalogueByteLength &&
...
export async function validateSnapshot(value: unknown): Promise<SnapshotValidation> {
  ...
  const { meta } = value;
  if (
    !isString(meta.datasetId) ||
    !isUri(meta.sourceRepository) ||
    !isDate(meta.dataAsOf) ||
    !(meta.previousFingerprint === null || typeof meta.previousFingerprint === 'string') ||
    !isUri(meta.assetBaseUrl) ||
    !isRecord(meta.scope) ||
    meta.scope.policy !== 'positive-evidence/current-known' ||
    meta.scope.allLocalitiesComplete !== false ||
    meta.scope.absenceIsNotEvidence !== true ||
    !Array.isArray(meta.licenceRefs) ||
  ...
  if (
    !allRecords(localizations) ||
    !allRecords(localSets) ||
    !allRecords(setEditions) ||
    !allRecords(works) ||
    !allRecords(items) ||
    !allRecords(assets) ||
    !hasUniqueIds(localizations, 'localizationId') ||
    !hasUniqueIds(localSets, 'localSetId') ||
    !hasUniqueIds(setEditions, 'setEditionId') ||
    !hasUniqueIds(works, 'workId') ||
    !hasUniqueIds(items, 'itemId') ||
    !hasUniqueIds(assets, 'assetId')
  ) {
    return { ok: false, reason: 'invalid' };
  }
  ...
  if (
    localizations.some(
      (row) =>
        !isString(row.locality) ||
        !isString(row.languageTag) ||
    ...
    localSets.some(
      (row) =>
        !isString(row.localSetId) ||
        !isString(row.locality) ||
        !isNullableText(row.localSetCode) ||
        !isNullableText(row.localSetName) ||
        typeof row.sortKey !== 'string',
    ) ||
    setEditions.some(
      (row) =>
        !isString(row.setEditionId) ||
        !isString(row.localSetId) ||
        !isString(row.localizationId) ||
        !isNullableText(row.localSetCode) ||
        !isNullableText(row.localSetName) ||
        typeof row.sortKey !== 'string',
    ) ||
  ...
  for (const item of items) {
    if (
      ...
      !isString(item.localizationId) ||
      !isString(item.setEditionId) ||
      !isString(item.localSetId) ||
      !isString(item.cardReleaseId) ||
      !isString(item.itemKind) ||
      !isString(item.progressClass) ||
      !isString(item.workMappingState) ||
      !isString(item.cardName) ||
      !isNullableText(item.localCardName) ||
      !isNullableText(item.localSetCode) ||
      !isNullableText(item.localSetName) ||
      !isNullableText(item.collectorNumber) ||
      !isNullableString(item.workId) ||
      !isNullableString(item.physicalPrintingId) ||
      !isNullableString(item.imageAssetId) ||
      !isString(item.imageScope) ||
      !itemKinds.has(item.itemKind) ||
      !progressClasses.has(item.progressClass)
    ) {
      return { ok: false, reason: 'invalid' };
    }
    ...
    if (
      (!workIsMapped && !unmappedWorkStates.has(item.workMappingState)) ||
      workIsMapped !== (item.workId !== null) ||
      (item.workId !== null && !workIds.has(item.workId))
    ) {
      return { ok: false, reason: 'invalid' };
    }
