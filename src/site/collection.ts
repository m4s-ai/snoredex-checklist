...
import type {
  PrivateStateLifecycle as BackupLifecycle,
  ImportPlan as BackupPlan,
  ExportedBackup,
  RecoveryRecordsBackup,
} from './state/backup.js';
type BackupExport = ExportedBackup | RecoveryRecordsBackup;
type BackupReadState = Extract<ReturnType<BackupLifecycle['read']>, { ok: true }>['value'];
...
export async function startCollection(appRevision: string): Promise<void> {
