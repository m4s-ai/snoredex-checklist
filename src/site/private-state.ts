import type { CollectionStatus } from './state/domain.js';

export interface PrivateStateRead {
  readonly readable: boolean;
  readonly hasActiveState: boolean;
  readonly statuses: ReadonlyMap<string, CollectionStatus>;
}

type AuthorityReader = typeof import('./state/authority.js').readStateAuthority;
...
export async function readPrivateState(
  ...
  try {
    ...
    if (active !== undefined && active.items.some((item) => !knownTrackableItemIds.has(item.itemId))) {
      return { readable: false, hasActiveState: true, statuses: new Map() };
    }
