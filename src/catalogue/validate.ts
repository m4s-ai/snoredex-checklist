import { canonicalize } from '../site/canonical-json.ts';
...
function indexBy<T>(rows: readonly T[], key: (row: T) => string, errors: Set<CatalogueErrorCode>): Map<string, T> {
...
function validateSemantics(catalogue: Catalogue): CatalogueErrorCode[] {
  ...
  const westSpanish = [...localizations.values()].find((row) => row.locality === 'WEST' && row.languageTag === 'es-ES');
  ...
  const releaseMappings = new Map<string, { setEditionId: string; workId: string | null; workMappingState: string }>();
...
export function validateCatalogueFixture(value: unknown): ValidationResult {
  ...
  for (const rawCase of value.reconciliationCases) {
    ...
    const cardinalityIsValid =
      ...
      (entry.changeKind === 'rekey-1:1' && entry.fromItemIds.length === 1 && entry.toItemIds.length === 1) ||
      (entry.changeKind === 'retired-1:0' && entry.fromItemIds.length === 1 && entry.toItemIds.length === 0) ||
      (entry.changeKind === 'split-1:N' && entry.fromItemIds.length === 1 && entry.toItemIds.length > 1) ||
      (entry.changeKind === 'merge-N:1' && entry.fromItemIds.length > 1 && entry.toItemIds.length === 1) ||
    ...
    const expected = RECONCILIATION_EXPECTATIONS[entry.changeKind as keyof typeof RECONCILIATION_EXPECTATIONS];
    if (
      !expected ||
