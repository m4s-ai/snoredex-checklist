...
test("fails closed on dangling references and invalid item classes", async () => {
  ...
  assert.deepEqual(await validateSnapshot(dangling), { ok: false, reason: "invalid" });

  const invalidClass = structuredClone(fixture.catalogue);
  invalidClass.items[0].itemKind = "research-placeholder";
  assert.deepEqual(await validateSnapshot(invalidClass), { ok: false, reason: "invalid" });
});
...
test("fails closed on orphan editions and duplicate physical printings", async () => {
  ...
  assert.deepEqual(await validateSnapshot(orphanEdition), { ok: false, reason: "invalid" });
  ...
  assert.deepEqual(await validateSnapshot(duplicatePrinting), { ok: false, reason: "invalid" });
});
...
test("fails closed on duplicate locality-language projections", async () => {
  ...
  assert.deepEqual(await validateSnapshot(duplicateProjection), { ok: false, reason: "invalid" });
});
