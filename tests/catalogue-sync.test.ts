...
function migrationFor(catalogue: JsonObject, targetFingerprint: string): Uint8Array {
  const firstItemId = catalogue.items[0].itemId as string;
  return new TextEncoder().encode(
    JSON.stringify({
      meta: {
        schema: 'snoredex-collector-migrations',
        schemaVersion: '1.1.0',
        fromFingerprint: `sha256:${'0'.repeat(64)}`,
...
test('stages and commits a validated catalogue plus matching lock', async () => {
  ...
  try {
    ...
    assert.equal(result.lock.migrationByteLength, request.migrationBytes.byteLength);
