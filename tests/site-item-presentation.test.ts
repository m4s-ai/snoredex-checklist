...
test("keeps item presentation labels explicit and distinct", () => {
  ...
  assert.equal(imageScopeLabel({ ...research, imageScope: "card-release" }, true), "Card-release placeholder (broader release)");
});
...
test("only publishes safe external links from untrusted snapshot fields", () => {
  ...
  assert.deepEqual(linkValues(["https://example.test/a", "javascript:alert(1)", 42]), ["https://example.test/a"]);
});
