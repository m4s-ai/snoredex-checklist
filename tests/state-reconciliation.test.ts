...
function migration(fromFingerprint: string, toFingerprint: string, transitions: readonly ReconciliationTransition[]): ReconciliationMigration {
...
test("reconciles retained and explicit one-to-one state without mutation", () => {
  ...
  if (!result.ok) return;
  assert.deepEqual(source.items[0], { itemId: oldA, status: "have", quantityOwned: 2, quantityOrdered: 0, note: "private" });
  assert.deepEqual(result.value.state.items, [{ itemId: targetA, status: "have", quantityOwned: 2, quantityOrdered: 0, note: "private" }]);
...
test("fails closed for split, merge, unresolved and unaccounted records", () => {
  ...
  const merge = reconcilePrivateState(state(oldFingerprint, [
  ...
  const unaccounted = reconcilePrivateState(state(oldFingerprint, [{ itemId: oldC, status: "skip", quantityOwned: 0, quantityOrdered: 0 }]), targetFingerprint, {
    migrations: [migration(oldFingerprint, targetFingerprint, [transition(oldA, [targetA], "rekey-1:1", "preserve", "one-to-one-preserve")])],
  });
...
test("rejects unknown, future and incomplete chains before mutation", () => {
  const unsupported = reconcilePrivateState(state(oldFingerprint, [{ itemId: oldA, status: "skip", quantityOwned: 0, quantityOrdered: 0 }]), targetFingerprint, {
    migrations: [],
  });
...
test("follows a deterministic multi-step chain and is idempotent", () => {
  ...
  if (first.ok) assert.deepEqual(first.value.state.items, [{ itemId: targetC, status: "ordered", quantityOwned: 0, quantityOrdered: 3 }]);
});
