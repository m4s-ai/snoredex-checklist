...
test('round-trips canonical public criteria', () => {
  const criteria = {
    localization: 'west-es',
    q: 'Snorlax & friends',
    kind: 'verified-printing' as const,
...
test('rejects the whole query when one criterion is invalid', () => {
  assert.deepEqual(parseQuery('?localization=west-es&q=ok&unknown=x', ids), {
    ok: false,
    recoverableLocalization: 'west-es',
  });
  assert.deepEqual(parseQuery('?localization=west-es&status=maybe', ids), {
    ok: false,
    recoverableLocalization: 'west-es',
  });
...
test('enforces raw and decoded query length boundaries', () => {
  const maximumLocalization = 'x'.repeat(4096 - 'localization='.length);
  assert.deepEqual(parseQuery(`?localization=${maximumLocalization}`, new Set([maximumLocalization])), {
    ok: true,
    criteria: { localization: maximumLocalization },
  });
  const overlongLocalization = `${maximumLocalization}x`;
  assert.deepEqual(parseQuery(`?localization=${overlongLocalization}`, new Set([overlongLocalization])), { ok: false });

  assert.deepEqual(parseQuery(`?q=${'x'.repeat(120)}`, ids), {
    ok: true,
    criteria: { q: 'x'.repeat(120) },
  });
  assert.deepEqual(parseQuery(`?q=${'x'.repeat(121)}`, ids), { ok: false });
});

test('enforces the decoded search term boundary', () => {
  const twelveTerms = Array.from({ length: 12 }, () => 'x').join(' ');
  assert.deepEqual(parseQuery(`?q=${encodeURIComponent(twelveTerms)}`, ids), {
    ok: true,
    criteria: { q: twelveTerms },
  });
  assert.deepEqual(parseQuery(`?q=${encodeURIComponent(`${twelveTerms} x`)}`, ids), { ok: false });
});
