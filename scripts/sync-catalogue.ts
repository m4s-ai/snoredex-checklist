import { MAX_CATALOGUE_BYTES, SYNC_ERROR_CODES, syncCataloguePair, type SyncErrorCode } from '../src/catalogue/sync.ts';
...
async function fetchArtifact(url: string): Promise<Uint8Array> {
  ...
  if (!response.ok) throw new Error('SYNC_TRANSACTION_FAILED');
  return readResponse(response);
}

async function main(): Promise<void> {
  const artifactUrl = required('--artifact-url');
  const migrationArtifactUrl = required('--migration-artifact-url');
  const bytes = await fetchArtifact(artifactUrl);
  const migrationBytes = await fetchArtifact(migrationArtifactUrl);
  const result = await syncCataloguePair({
    ...
    migrationExpectedByteSha256: required('--migration-byte-sha256'),
    migrationBytes,
